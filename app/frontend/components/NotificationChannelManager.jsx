...
const NotificationChannelManager = () => {
  ...
  const loadChannels = async () => {
    setLoading(true)
    try {
      const data = await userNotificationService.getNotificationChannels()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingChannel) {
        ...
        await userNotificationService.createNotificationChannel(formData)
      }
      closeModal()
      loadChannels()
    } catch (error) {
  ...
  const handleToggle = async (channelId) => {
    try {
      await userNotificationService.toggleNotificationChannel(channelId)
      loadChannels()
    } catch (error) {
  ...
  return (
    <div>
      <Group position="apart" mb="md">
        <Text size="xl" weight={700}>Notification Channels</Text>
        <Button 
        ...
        {channels.map((channel) => (
          <Card key={channel.id} shadow="sm" padding="lg" radius="md" withBorder>
            <Group position="apart" mb="xs">
              <Group>
                <Text size="lg">{getChannelIcon(channel.channelType)}</Text>
                <div>
                  <Text weight={500}>{channel.name || getChannelTypeLabel(channel.channelType)}</Text>
                  <Text size="sm" color="dimmed">{channel.channelDescription}</Text>
                </div>
              </Group>
              <Badge color={channel.enabled ? 'green' : 'red'}>
            ...
            <Group position="apart" mt="md">
              <Group spacing="xs">
                <ActionIcon
                  color={channel.enabled ? 'red' : 'green'}
                  onClick={() => handleToggle(channel.id)}
                  title={channel.enabled ? 'Disable' : 'Enable'}
                >
                  {channel.enabled ? <IconToggleLeft size={16} /> : <IconToggleRight size={16} />}
                </ActionIcon>
                <ActionIcon
                  color="blue"
